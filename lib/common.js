...
exports.getFullConfig = (config) => {
    ...
    // Object.keys(argv).concat(Object.entries(config)).forEach(entry => {
    //     const lowerCaseKey = entry[0].toLowerCase();
    //     const value = entry[1];
    ...
    //         'threadTimeLimit',
    //         'waitForFileExist'
    //     ]).find(k => k.toLowerCase() === lowerCaseKey)

    //     if (mappedKey) {
    //         fullConfig[mappedKey] = value;
    //     }
    // });
